...
module UFeeling
  module Services
    ...
    class AddVideo
      ...
      def parse_url(input)
        if input.success?
          from_param_fild = input[:video_url].include?('youtu.be/') || input[:video_url].include?('youtube.com/shorts/')
          video_id = from_param_fild ? id_from_param_fild(input[:video_url]) : id_query_field(input[:video_url])
          Success(video_id:)
        else
      ...
      def id_from_param_fild(video_url)
